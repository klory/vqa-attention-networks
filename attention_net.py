...
class Attention_net(nn.Module):
    ...
    def forward(self, img_features, que_features):
        ...
        for i in range(self.att_num):
            if i%2 == 0:
                img, que, que_att = self._modules['att{}'.format(i)](img, que) # que_att: [N, 49, 22], image-guided question attentions
            else:
                que, img, img_att = self._modules['att{}'.format(i)](que, img) # img_att: [N, 22, 49], question-guided image attentions
...
class Attention_net_parallel(nn.Module):
    def __init__(self, block_num=49, word_num=22, img_size=1024, vocab_size=15881, embed_size=512, att_num=6, output_size=3000):
        super(Attention_net_parallel, self).__init__()
        self.img_emb = nn.Linear(img_size, embed_size, bias=True)
        self.que_emb = nn.Embedding(vocab_size, embed_size)
    ...
    def forward(self, img_features, que_features):
        ...
        Cv = self.fc_Wbv(img) # N, 49, 512
        Cq = self.fc_Wbv(que) # N, 22, 512
        C = F.tanh(torch.matmul(Cq, Cv.transpose(1,2))) # [N, 22, 49]
        C = F.dropout(C)

        img_ = self.fc_Wv(img) # N, 49, 512
        que_ = self.fc_Wq(que) # N, 22, 512

        Hv = F.tanh(img_ + (torch.matmul(que_.transpose(1, 2), C)).transpose(1,2)) # N, 49, 512
        Hv = F.dropout(Hv)
        av = F.softmax(self.fc_Whv(Hv), dim=1) # N, 49, 1
        av_ = av.permute(0, 2, 1) # N, 1, 49
