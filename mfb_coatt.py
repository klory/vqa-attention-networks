...
class MFB(nn.Module):
    def __init__(self, cfg):
        ...
        self.img_conv1d = nn.Linear(cfg.img_feature_channel, 5000)

        # co-attention conv layers
        self.co_att_conv1 = nn.Conv2d(cfg.img_feature_dim, 512, [1,1])
        # number of question for glimpse: 2
        ...
        self.img_proj2 = nn.Linear(cfg.img_feature_dim, 5000)
    ...
    def forward(self, img_features, questions, is_training=True):
        ...
        img_projed = self.img_conv1d(img_features) # N, D, L, 1: N 2048 100 1 --> N, 5000, 100, 1
        ques_projed = self.ques_proj1(ques_att_feature) # N 5000

        # fusion: element-wise product
        fusion_feature = img_projed * ques_projed # N, 5000, 100, 1
        ...
        fusion_feature = torch.sum(fusion_features, 3) # N, 100, 1000, 1
        # power normalization and normalization
        fusion_feature = fusion_feature.permute(0, 2, 1, 3) # N, 1000, 100, 1
        fusion_pow_normed = torch.sqrt(F.relu(fusion_feature)) - torch.sqrt(F.relu(-fusion_feature)).view(self.cfg.batch_size, -1)
        fusion_normed = F.normalization(fusion_pow_normed).view(self.cfg.batch_size, 1000, self.cfg.img_feature_dim, -1) # N, 1000, 100, 1
        ...
        att_normed = F.normalization(att_pow_normed).view(self.cfg.batch_size, 1000, self.cfg.img_feature_dim, -1) # N, 1000, 100, 1
